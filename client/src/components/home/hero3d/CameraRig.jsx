...
// Phase 7.4 — lowered from 0.12 and run through easePointer() below: the
...
// Phase 7.3 — composition only. At the Phase 7.1/7.2 defaults (fov 38,
...
// content grid (verified against real measured DOM gaps — see the Phase
// 7.3 report) without moving the grid, the gallery, the panels, or the
// ball's own world position (CricketBall.jsx is unchanged).
const CAMERA_DISTANCE = 13
const CAMERA_FOV = 36
const LOOK_AT_Y = -2.6

// Phase 7.3 — narrow (phone-width) viewports get a bit more distance so
...
/**
 * Phase 7.1 foundation only: a slow idle "breathe" (sine drift on
 * position) plus a tiny mouse-parallax offset sourced from Phase 4's
 ...
 * (the same signal Phase 4 uses everywhere else), so only the breathe
