...
export default function SiteFooter() {
  return (
    <footer className="relative border-t border-emerald-400/10 bg-loc-dark/60 px-6 py-6 lg:px-10">
      <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-center gap-x-8 gap-y-4 sm:justify-between">
        ...
        <nav aria-label="Footer" className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-emerald-100/60">
          {LOC_LINKS.map((link) => (
        ...
        <p className="shrink-0 text-xs text-emerald-100/40">© {new Date().getFullYear()} LOC — Lord Of Cricket. All rights reserved.</p>
      </div>
    </footer>
  )
}
