  ...
  return (
    <div className="flex gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="min-w-0">
        ...
        {when && (
          <p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-emerald-300/80">
            {when}
  ...
  if (earned.length === 0 && !next) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-white/10 bg-white/5 px-6 py-10 text-center">
        ...
        <p className="text-sm text-slate-300">No career milestones reached yet — they appear as finalized matches add up.</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {next && (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Next Milestone</p>
          <p className="mt-1 text-sm font-bold text-white">{next.title}</p>
          <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-white/10">
            <div className="h-full rounded-full bg-emerald-400" style={{ width: `${nextPct}%` }} />
          </div>
