...
import { useAdminPartners } from '../../hooks/useAdminPartners.js'
  ...
  return (
    <div className="min-h-screen bg-emerald-950 px-6 py-12">
      <div className="mx-auto flex max-w-3xl flex-col gap-10">
        <h1 className="text-3xl font-bold text-white">Manage Dealing Partners</h1>
        ...
        >
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-emerald-100/70">Logo</label>
            <input
              type="file"
              accept="image/*"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-emerald-100/70">Website (optional)</label>
            <input
              type="url"
              value={websiteUrl}
          ...
          >
            ...
            {submitting ? 'Uploading…' : 'Upload Partner'}
          </button>
        </form>

        <div className="flex flex-col gap-4">
          <h2 className="text-lg font-semibold text-white">
            Current Partners {loading ? '' : `(${partners.length})`}
          </h2>

          {loading ? (
            <p className="text-emerald-100/60">Loading…</p>
          ) : partners.length === 0 ? (
            <p className="text-emerald-100/60">No partners yet — upload one above.</p>
          ) : (
              ...
              {partners.map((partner) => (
                ...
                >
                  <img
                    ...
                    alt={partner.name}
                    className="h-32 w-full object-contain bg-white/5 p-3"
                  />
                  <button
                    type="button"
